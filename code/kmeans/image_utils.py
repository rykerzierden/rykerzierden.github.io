...
"""This module contains useful functions needed in the k means and run k means files, including allowing the reading and writing of ppm files"""
import random
...
def save_ppm(filename, image):
    ...
    for i in range(height):
        for j in range(width):
            print(image[j][i][0],image[j][i][1],image[j][i][2], file=out_file)
        print('\n', file=out_file)
